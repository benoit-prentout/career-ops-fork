# Career-Ops -- AI Job Search Pipeline (Qwen Code)

## Origin

This system was built and used by [santifer](https://santifer.io) to evaluate 740+ job offers, generate 100+ tailored CVs, and land a Head of Applied AI role. The archetypes, scoring logic, negotiation scripts, and proof point structure all reflect his specific career search in AI/automation roles.

The portfolio that goes with this system is also open source: [cv-santifer](https://github.com/santifer/cv-santifer).

**It will work out of the box, but it's designed to be made yours.** If the archetypes don't match your career, the modes are in the wrong language, or the scoring doesn't fit your priorities -- just ask. You (AI Agent) can edit the user's files. The user says "change the archetypes to data engineering roles" and you do it. That's the whole point.

## Platform

You are running in **Qwen Code**. This file provides the same session context as `CLAUDE.md` does for Claude Code, with Qwen-specific adaptations.

- The `.qwen/` directory contains Qwen-native skill definitions and commands
- The `.claude/` directory is preserved for backward compatibility with Claude Code users
- The `modes/` directory is **shared** between both platforms
- All functionality is identical regardless of which CLI you use
...
- `CLAUDE.md`, `QWEN.md`, `*.mjs` scripts, `dashboard/*`, `templates/*`, `batch/*`
- `.claude/` and `.qwen/` platform files
...
AI-powered job search automation built on Qwen Code: pipeline tracking, offer evaluation, CV generation, portal scanning, batch processing.
...
When using Qwen Code, the following slash commands are available (defined in `.qwen/commands/` and `.qwen/skills/`):
...
**Note:** The skill in `.qwen/skills/career-ops/skill.md` is the Qwen-native skill definition. The `modes/*` files are shared between both platforms.
...
>
> Tip: Having a personal portfolio dramatically improves your job search. If you don't have one yet, the author's portfolio is also open source: github.com/santifer/cv-santifer — feel free to fork it and make it yours."

Then suggest automation:
> "Want me to scan for new offers automatically? I can set up a recurring scan every few days so you don't miss anything. Just say 'scan every 3 days' and I'll configure it."

If the user accepts, use the `/loop` skill (if available) to set up a recurring `/career-ops scan`. If that's not available, suggest adding a cron job or remind them to run `/career-ops scan` periodically.
...
**Exception for batch workers (`qwen -p` or `claude -p`):** Playwright is not available in headless pipe mode. Use WebFetch as fallback and mark the report header with `**Verification:** unconfirmed (batch mode)`. The user can verify manually later.
...
- Node.js (mjs modules), Playwright (PDF + scraping), YAML (config), HTML/CSS (template), Markdown (data), Canva MCP (optional visualCV)
